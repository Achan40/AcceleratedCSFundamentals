...
int GridGraph::countEdges() const {
  ...
  std::unordered_set<IntPairPair> uniEdge;
  std::unordered_set<IntPair> isoPoints;

  // Loop over the key-value pairs
  for (const auto& keys : adjacencyMap) {
    const auto& p1 = keys.first;
    // Neighbor to p1
    const auto& p1_neighbor = keys.second;
    // If points are isolated insert to isoPoints set
    if (p1_neighbor.empty()) {
      isoPoints.insert(p1);
    } else {
      for (const auto& p2 : p1_neighbor ) {
        IntPairPair edge;
        if (p1 < p2) {
...
void GridGraph::removePoint(const IntPair& p1) {
  ...
  for (auto np : originalNeighbors) {
    adjacencyMap.at(np).erase(p1);
  }
  // =======================================================================
  // TODO: Your code here!
  // =======================================================================
