...
void DisjointSets::bfs(int i, int n, int m, int edges[][2]) {
  ...
  for (int d = 1; d < m; d++) {
    ...
    for (int j = 0; j < m; j++) {
      ...
      if (explored.count(edges[j][0]) > 0 && explored.count(edges[j][1]) == 0) {
        
        frontier.push(edges[j][1]);
      ...
      } else if (explored.count(edges[j][0]) == 0 && explored.count(edges[j][1]) > 0) {
        
        frontier.push(edges[j][0]);
