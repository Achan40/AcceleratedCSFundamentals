...
static void treeFactory(GenericTree<int>& tree) {
  ...
  //clean out old tree
  tree.createRoot(4);
  //make root node and assign as NodeA
  auto nodeA = tree.getRootPtr();
  //nodeB has value 8, create children as well
  auto nodeB = nodeA->addChild(8);
  nodeB->addChild(16)->addChild(42);
  nodeB->addChild(23);
  //nodeC has value 15
  auto nodeC = nodeA->addChild(15);
  //      *****************************************************
  //                           EXERCISE 1
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  std::queue<TreeNode*> nodeZ;
  nodeZ.push(rootNodePtr);

  while(!nodeZ.empty()) {
    // If the node exists, it may have children pointers. Let's iterate
    // through the childrenPtrs vector and push copies of those pointers
    // onto the exploration queue.
    for (auto childPtr : nodeZ.front()->childrenPtrs) {
      nodeZ.push(childPtr);
    }
    // Make a copy of the top pointer on the queue, then pop it to decrease the queue
  	results.push_back(nodeZ.front()->data);
    nodeZ.pop();
  }
  //      *****************************************************
  //                           EXERCISE 2
