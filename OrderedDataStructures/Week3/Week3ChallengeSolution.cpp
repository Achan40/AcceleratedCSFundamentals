...
int countChild(Node *n) {
  int totalH = 0, leftH = 0, rightH = 0;
  
  if (n->left) {
    leftH++;
    leftH += countChild(n->left);
  }
  
  if (n->right) {
    rightH++;
    rightH += countChild(n->right);
  }
  
  leftH >= rightH ? totalH = totalH + leftH : totalH = totalH + rightH;
  n->height = totalH;
  return totalH;
}
