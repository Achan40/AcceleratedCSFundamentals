...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  } else if (newNode->data >= back()) {
    pushBack(newNode->data); // Check the last node and insert there if newNode > tail
  } else if (newNode->data <= front() ) {
    pushFront(newNode->data);// Check the first node and insert there if newNode < head
  } else {
...
//check if left and right lists are sorted, and if they are not, sort them. Functions defined in LinkedList.h
  if (!right.isSorted()) {
    right.mergeSort();
  }
  ...
  while ((left.size() + right.size()) > 0) {
    if (left.empty()) {
      ...
      right.popFront();
      //edge case left empty

    } else if (right.empty()) {
      merged.pushBack(left.front());
      left.popFront();
      //edgecase right empty

    } else if (left.front() <= right.front()) {
      merged.pushBack(left.front());
      left.popFront();
      //if left less than right, add to merge
